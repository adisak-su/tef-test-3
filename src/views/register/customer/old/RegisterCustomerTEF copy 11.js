...
const RegisterCustomerTEF = () => {
  ...
  function genIniDataBlock() {
    let tmpDataBlock = [];
    initDataBlock1.forEach((element) => {
      let item = { id: element.id, type: element.type, value: element.value, required:element.required };
      tmpDataBlock.push(item);
    });
    initDataBlock2.forEach((element) => {
      let item = { id: element.id, type: element.type, value: element.value, required:element.required };
      tmpDataBlock.push(item);
    });
    initDataBlock3.forEach((element) => {
      let item = { id: element.id, type: element.type, value: element.value, required:element.required };
      tmpDataBlock.push(item);
    });
    initDataBlock4.forEach((element) => {
      let item = { id: element.id, type: element.type, value: element.value, required:element.required };
      tmpDataBlock.push(item);
    });
    return tmpDataBlock;
